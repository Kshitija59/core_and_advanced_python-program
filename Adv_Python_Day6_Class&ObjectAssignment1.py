...
class Library:
    ...
    def borrow_book(self, book):
        ...
        if book in Library.all_books:
            ...
            print(f"{self.name} successfully borrowed '{book}'")
        else:
    ...
    def return_book(self, book):
        ...
        if book in self.borrowed_books:
            ...
            Library.total_books += 1
            print(f"{self.name} successfully returned '{book}'")
        else:
            print(f"{self.name} hasn't borrowed '{book}'")

    @classmethod
    def view_library_books(cls):
        """Display total number of books and list all available books."""
        print(f"Total books in library: {cls.total_books}")
        print("Books currently available in library:")
...
# Initializing the library with some books
...
member2.return_book('The Great Gatsby')  # Bob didn't borrow this book

# Viewing available books after actions
Library.view_library_books()
